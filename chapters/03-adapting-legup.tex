...
As described in \cref{subsec:tbgen}, a way to automatically include testcases into the testbench is implemented. This parameter provides the filename of a file containing testcases for the testbench. If the parameter is not set, no testcases will be added to the testbench.
...
One of the main issues with using LegUp for ASIC implementations, is that it generates a global memory-controller for passing data between modules. Both to decrease the overhead area, and to simplify the generated design, it is desirable to avoid this memory controller. A simple solution to this, is to set the parameter \textit{LOCAL\_RAMS} to 1. This will avoid creating the global memory controller, as long as there are no variables used by multiple modules (global variables), or pointers that cannot be connected to a single module after points-to analysis. 
...
\caption{\label{fig:assigningoutputs}4 figures}
 \end{figure}
